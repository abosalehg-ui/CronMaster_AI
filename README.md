...
| 🌍 **`--lang en`** | مخرجات إنجليزية **كاملة** — بما فيها تشخيص الأخطاء ورسائل الإصلاح؛ العربية تبقى الافتراضية بلا أي تغيير |
...
## 🔧 ما الجديد في 3.1 — جولة تنفيذ المراجعة

نُفِّذت خطة الإصلاح الكاملة في [`docs/مراجعة-CronMaster_AI.md`](docs/مراجعة-CronMaster_AI.md).

| التغيير | ماذا يعني لك |
|---|---|
| 🔕 **تهدئة التصعيد** | رسالة «رفع المهلة لم يعد مجدياً» كانت تصلك في **كل** دورة مراقبة؛ صارت تحترم `alert_cooldown_hours` مثل بقية التنبيهات |
| 🧾 **تحقق أنواع الإعدادات** | `{"telegram_chat_id": 123456}` بلا علامتي تنصيص كان يوقف التنبيهات بصمت، و`{"lang": 5}` كان يُسقط الأداة — الآن تُصحَّح القيم ويعرضها `doctor` |
| 🛡️ **حجب الأسرار** | نص الخطأ يُنقّى من الرموز وكلمات المرور وسلاسل الاتصال قبل إرساله إلى المصنّف الذكي |
| ⚡ **أداء الإحصائيات** | `stats` وتقرير HTML كانا يكلّفان 5 استعلامات لكل مهمة؛ صارا استعلاماً تجميعياً واحداً + واحداً مسقوفاً لكل مهمة، و`record_cycle` تلتزم مرة واحدة بدل مرة لكل صف |
| 🌍 **`--lang en` كامل** | تشخيص الأخطاء ورسائل الإصلاح كانت عربية داخل الكود فتظهر في الواجهة الإنجليزية |
| 🖥️ **عرض بشري** | `monitor` و`list` و`fix` و`history` تعرض ملخصاً مقروءاً على الطرفية، وJSON عند إعادة التوجيه أو مع `--json` |
| 🧯 **لا traceback** | أي استثناء غير متوقع يصلك كسطر عربي واضح مع مسار ملف السجل |
| 🗂️ **`policies.py`** | سياسات الإصلاح الخمس خرجت من المنسّق فصارت قابلة للاختبار والتوسيع منفردة |

### ⚠️ تغييرات سلوك تستحق الانتباه

| التغيير | الإجراء المطلوب |
|---|---|
| قنوات webhook على **`http://` مرفوضة** افتراضياً | لشبكة داخلية: `"allow_insecure": true` على القناة أو `allow_insecure_webhooks: true` |
| الحد الأدنى صار **Python 3.9** | كان `>=3.8` معلناً بلا اختبار، و`zoneinfo` (فترة الهدوء بالمنطقة الزمنية) تحتاج 3.9 أصلاً |
| `restore --backup` يقبل **مسارات داخل مجلد النسخ فقط** | استخدم `--list` لعرض المتاح؛ المسار الخارجي يُرفض برسالة واضحة |
| `StateManager.record_fix` لم تعد تحفظ تلقائياً | يخص من يستدعي المكتبة برمجياً فقط: نادِ `save()` بعد التسجيل |
| `mean_time_between_failures` تُحسب على آخر 50 ملاحظة | كانت تسحب كل الصفوف بلا حد؛ النتيجة أدق للسلوك الحديث وأرخص كثيراً |
...
الأوامر `monitor` و`list` و`fix` و`history` تعرض **ملخصاً بشرياً** حين تكون
المخرجات طرفية، و**JSON** حين لا تكون (أنبوب، إعادة توجيه، مهمة مجدولة) — فلا
تنكسر السكربتات القائمة. `--json` يفرض JSON دائماً.

```bash
python3 CronMaster_AI.py monitor          # ملخص مقروء على الطرفية
python3 CronMaster_AI.py monitor --json   # JSON صريح
python3 CronMaster_AI.py monitor > out.json   # JSON تلقائياً
```
...
- لا يتكرر نفس التنبيه (نفس المهمة ونفس نوع الخطأ) قبل `alert_cooldown_hours`. **هذا يشمل رسائل التصعيد** (الإصلاح غير المجدي، التباطؤ، قاطع الدائرة) لا تنبيهات الفشل وحدها.
...
> **`https` فقط افتراضياً.** نص التنبيه يحمل أسماء المهام وأوصاف الأخطاء، وعنوان `http://` يعني عبوره الشبكة بلا تشفير فيُرفض. لشبكة داخلية: `"allow_insecure": true` على القناة، أو `allow_insecure_webhooks` عاماً.
...
- **حجب الأسرار قبل الإرسال:** نص الخطأ يمرّ على `redact()` فتُستبدل الرموز وكلمات المرور وترويسات `Authorization` وسلاسل الاتصال ومعاملات URL الحساسة وJWT بـ `<محجوب>`. المصنّف يحتاج **شكل** الخطأ لا قيمة السر.
...
> **مع ذلك، اعرف ما يغادر جهازك:** الحجب يعتمد على أنماط معروفة، ولا يمكنه ضمان التقاط كل شكل سرّ. إن كانت مهامك تطبع أسراراً بصيغ غير قياسية في أخطائها، أبقِ الميزة معطّلة.
...
├── format.py                  # تنسيق القيم ومحاذاة الطرفية (مشترك)
...
├── policies.py                # سياسات الإصلاح الخمس
├── core.py                    # المنسّق
...
المفاتيح المجهولة يُحذَّر منها في السجل وتظهر في `doctor`. والقيم ذات النوع الخاطئ
تُصحَّح بهدوء بدل إسقاط الأداة — `{"telegram_chat_id": 123456}` بلا علامتي تنصيص
يعمل، و`doctor` يعرض ما صُحِّح تحت «مفاتيح صُحِّحت قيمتها».
...
| `allow_insecure_webhooks` | `CRONMASTER_ALLOW_INSECURE_WEBHOOKS` | `false` | السماح بقنوات webhook على `http` بلا تشفير |
...
يعمل فوق `tmp_path` وواجهة خلفية مزيفة و`anthropic` مزيف. وعزل الإعدادات آلي
عبر `Config.snapshot()` / `Config.restore()`، فأي إعداد جديد يدخل العزل بلا
تسجيل يدوي.

`tests/test_multi_cycle.py` يفحص ما لا تكشفه دورة واحدة: عدم تكرار التنبيهات،
توقّف العدّادات عند سقوفها، عدم نمو ملف الحالة بلا حد، وعدم تكرار صفوف السجل.
